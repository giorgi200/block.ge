<?php
    ...
    if(isset($_FILES['slider_imgs'])){
        ...
        for($i = 0; $i <  count($_FILES['slider_imgs']['name']); $i++){
            ...
            $image_names = date("Y_m_d_H_i_s") . "_slider_$i.$file_exts";
        }
    ...
    if(empty($errors)){
        ...
        $query = "INSERT INTO `investments` (`category_id`, `label`, `label_ka`, `img`, `progress`, `value`, `location`, `location_ka`, `about_ka`, `about_en`, `web_link`, `web_name`, `icon`, `slide_imgs`, `Shareholding`) VALUES ('$category', '$label_en', '$label_ka', '$image_name', '$complete', '$value', '$location_en', '$location_ka', '$about_ka', '$about_en', '$web_link', '$web_title', '$icon', '$slider_imgs_decode', '$Shareholding' )";
        $pdo -> query($query);
